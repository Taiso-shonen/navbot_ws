...
class Nav2Mapping(Node): 
    ...
    def goal_generator(self):
        ...
        for cell in unknown_cells:
            y, x = cell
            self.is_goal_valid_= self.check_goal_cells(x, y, mixed_cells)
            if self.is_goal_valid_:
                ...
                self.publish_frontier_markers(self.robot_cells_, 0.0, 0.0, 1.0, 1)
    ...
    def check_goal_cells(self, x, y, mixed_cells):
        ...
        mixed_cells_set = set(map(tuple, mixed_cells))

        result = robot_cells_set.issubset(mixed_cells_set)
        return result
